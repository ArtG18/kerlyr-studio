...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-sm border-b border-rose-100">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <img src="/gallery/Logo.jpg" alt="Kerlyr Studio" className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
            <div className="min-w-0">
              <p className="text-sm font-semibold text-gray-800 leading-none truncate">Kerlyr Studio</p>
              <p className="text-[10px] text-gray-400 tracking-widest uppercase hidden sm:block">Nails · Pestañas · Cejas</p>
            </div>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <button onClick={() => navigate('/login')} className="text-xs text-gray-500 hover:text-gray-700 transition-colors flex items-center gap-1 border border-gray-200 px-2.5 py-1.5 rounded-full hover:border-gray-300 flex-shrink-0">
      ...
      {scrolled && (
        <div className="fixed bottom-5 right-4 z-50 flex flex-col items-end gap-2">
          ...
          <button
            ...
            className={`fab-btn ${pulse ? 'fab-pulse' : ''} flex items-center gap-2 text-white font-bold text-sm px-4 py-3 rounded-2xl shadow-xl shadow-rose-300/50`}
          >
      ...
      {/* Botón flotante siempre visible en móvil (antes del primer scroll) */}
      {!scrolled && (
        <div className="fixed bottom-5 right-4 z-50 flex flex-col items-end gap-2 sm:hidden">
          <button
            onClick={() => navigate('/portal')}
            className={`fab-btn ${pulse ? 'fab-pulse' : ''} flex items-center gap-2 text-white font-bold text-sm px-4 py-3 rounded-2xl shadow-xl shadow-rose-300/50`}
          >
